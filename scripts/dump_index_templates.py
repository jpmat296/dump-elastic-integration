...
cts = es.cluster.get_component_template(name='logs-cisco_ise.log@package')
print(f"{len(cts['component_templates'])} component templates found.")

tmpls = es.indices.get_index_template(name='logs-cisco_ise.log')
...
with open("dev_tools_index.txt", "w") as outfile: 
    ...
    outfile.write("GET _component_template/*-cisco_ise.*?filter_path=*.name\n")
    outfile.write("GET _index_template/*-cisco_ise.*?filter_path=*.name\n")
